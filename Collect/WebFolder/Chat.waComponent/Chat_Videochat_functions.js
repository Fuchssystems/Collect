...
var peer = null; // peerjs peer object
var WebRTC_remoteVideo = document.querySelector('#component_Main_video_otherWebuser');
...
var Videochat_Call_Main = function (source_user_in_profile) {
	var Videochat_Call_Start = function (source_user_in_profile) {
		...
		if (OK_Flag) {
			...
			peer = new Peer(WAF_directory_currentUser.userName, {key: 'a58uyqxmtqp5vcxr'}); // key obtained by registering on peerjs.com
			peer.on('error', WebRTC_peer_onError_Callback);
...
var Videochat_Accept_Call_Main = function () {
		...
		var Videochat_Accept_Call_Accept = function () {
			if (videochat_current.status === 'videochat_status_incoming_call') { // not timed out during time to accept webcam use
				...
				// peerjs on answer call
				peer.on('call', function(call) {
				  // Answer the call, providing our mediaStream
				  call.answer(localMediaStream);
				  call.on('stream', WebRTC_Call_Callback);
				}); // end peer.on
...
var Videochat_Window_Close = function () {
	...
	WebRTC_Clear_Variables(); // clear WebRTC variables
...
var ws_videochat_status_channel_open = function (data_received) {
	...
	WebRTC_Call_Start(); // Call the other peer, providing our mediaStream
}; // end var ws_videochat_status_channel_open

var WebRTC_Call_Start = function () {
	// Call the other peer, providing our mediaStream
	var call = peer.call(videochat_current.other_Webuser_Name, localMediaStream);
	call.on('stream', WebRTC_Call_Callback);
}; // end var WebRTC_Call_Start

// callback for peer.call and the callback of the call event
var WebRTC_Call_Callback = function(stream) {
	WebRTC_remoteVideo.src =  window.URL.createObjectURL(stream);
}; // end var WebRTC_Call_Callback

// clear WebRTC variables
var WebRTC_Clear_Variables = function () {
	if (peer) peer.destroy();
	WebRTC_remoteVideo.src = '';
}; // end var WebRTC_Clear_Variables

// peerjs onError callback
var WebRTC_peer_onError_Callback = function(error) {
	alert('peerjs error: ' + err.type);
}; // end var WebRTC_peer_onError_Callback
